...
def cmd_config_patch(args: argparse.Namespace) -> int:
    ...
    if mqtt_disabled:
        ...
        if not pm:
            emit_envelope(command="config-patch", ok=False, error="CONFIG_PATCH_ARGS", payload={"detail": "--mqtt-enabled requires mqtt values (flags or config defaults)"}, as_json=bool(getattr(args, "as_json", False)))
            return 1
        patch_obj["mqtt"] = pm
