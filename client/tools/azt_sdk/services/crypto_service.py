...
from tools.azt_client.crypto import gen_rsa_keypair_with_fingerprint, spki_fp_hex_from_private_key

CLIENT_ROOT = Path(__file__).resolve().parents[3]


def create_credentials(*, identity: str | None, identity_prefix: str) -> dict:
...
def key_fingerprint_from_private_key(*, key_path: str) -> str:
    return spki_fp_hex_from_private_key(Path(key_path))
