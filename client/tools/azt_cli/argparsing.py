...
def build_parser(handlers: argparse.Namespace) -> argparse.ArgumentParser:
    ...
    spatch.add_argument("--listener-key-pem", dest="listener_key_pem", default="", help="Set patch.listener_key.public_key_pem from PEM file path")
    spatch.add_argument("--recorder-auth-key", dest="recorder_auth_key_path", default="", help="Set patch.recorder_auth_key from Ed25519 private key PEM path")
    ...
    spatch.add_argument("--mqtt-audio-rms-topic", default="", help="Set patch.mqtt.audio_rms_topic")
    spatch.add_argument("--mqtt-audio-rms-window-seconds", type=int, default=None, help="Set patch.mqtt.rms_window_seconds (1..3600)")
    ...
    spatch.add_argument("--stream-header-auto-record", action="store_true", help="Set patch.stream_header_flags.auto_record=true")
    spatch.add_argument("--stream-header-no-auto-record", action="store_true", help="Set patch.stream_header_flags.auto_record=false")
    spatch.add_argument("--stream-header-auto-decode", action="store_true", help="Set patch.stream_header_flags.auto_decode=true")
    spatch.add_argument("--stream-header-no-auto-decode", action="store_true", help="Set patch.stream_header_flags.auto_decode=false")
    ...
    sv.add_argument("--key", dest="key_path", default="", help="Listener private key PEM (required only for encrypted next-header mode)")
    ...
    sd.add_argument("--key", dest="key_path", required=True, help="Listener private key PEM")
    ...
    sb.add_argument("--firmware-key", dest="firmware_key_path", default="", help="Firmware signer private key PEM")
    ...
    sb.add_argument("--admin-key", dest="admin_key_path", default="", help="Admin Ed25519 private key PEM for OTA wake auth during --post")
    ...
    spost.add_argument("--admin-key", dest="admin_key_path", default="", help="Admin Ed25519 private key PEM for OTA wake auth")
    spost.add_argument("--wake-window-seconds", type=int, default=30, help="Requested OTA open window seconds (default: 30)")
    spost.add_argument("--wake-allow-self", dest="wake_allow_self", action="store_true", default=True, help="Bind OTA wake to caller IP (default: true)")
    spost.add_argument("--no-wake-allow-self", dest="wake_allow_self", action="store_false", help="Disable self-IP binding; requires --wake-allowed-ip")
    spost.add_argument("--wake-allowed-ip", default="", help="Allowed uploader IPv4 when --no-wake-allow-self is used")
    ...
    sdh.add_argument("--key", dest="key_path", required=True, help="Listener private key PEM")
    ...
    sconf = sub.add_parser("configure-device", help="Apply signed config to device using admin/listener credentials")
    sconf.add_argument("--admin-creds-dir", required=False, default="", help="Path to admin credentials directory")
    sconf.add_argument("--listener-creds-dir", default="", help="Path to listener credentials directory (defaults to admin creds)")
    sconf.add_argument("--recorder-auth-creds-dir", default="", help="Path to recorder auth Ed25519 credentials directory/file (optional)")
    ...
    sconf.add_argument("--mqtt-enabled", action="store_true", help="Enable MQTT RMS telemetry using mqtt_* values (explicit opt-in)")
    sconf.add_argument("--enable-mqtt", dest="mqtt_enabled", action="store_true", help=argparse.SUPPRESS)
    sconf.add_argument("--mqtt-broker-url", default="", help="MQTT broker URL (e.g. mqtt://host:1883)")
    sconf.add_argument("--mqtt-username", default="", help="MQTT username")
    sconf.add_argument("--mqtt-password", default="", help="MQTT password")
    sconf.add_argument("--mqtt-audio-rms-topic", default="", help="MQTT topic for periodic RMS dBFS telemetry")
    sconf.add_argument("--mqtt-audio-rms-window-seconds", type=int, default=None, help="RMS window/publish period in seconds (1..3600)")
    ...
    sprobe.add_argument("--key", dest="key_path", default="", help="Admin signer private key PEM (used for certificate verification in trusted recording)")
    sprobe.add_argument("--auth-key", dest="auth_key_path", default="", help="Recorder-auth private key PEM used to sign stream challenge when required")
