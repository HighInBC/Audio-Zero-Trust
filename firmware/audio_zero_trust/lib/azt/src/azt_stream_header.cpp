...
bool build_header_prefix(StreamCtx& sc,
  ...
  {
    const uint8_t* nonce_bytes = reinterpret_cast<const uint8_t*>(stream_auth_nonce.c_str());
    if (!sha256_bytes(nonce_bytes, stream_auth_nonce.length(), sc.chain_nonce_hash)) {
      return false;
    }
  }
  ...
  dec_header += "\"payload_block_types\":{\"0\":\"pcm_audio\",\"1\":\"ed25519_checkpoint_signature\",\"2\":\"dropped_frames_notice\",\"3\":\"telemetry_snapshot\",\"126\":\"message\",\"127\":\"finalize_signature\"},";
  dec_header += "\"block_type_encoding\":\"msb_encryption_flag_v1\",";
  dec_header += "\"block_type_encrypted_mask\":128,";
  dec_header += "\"block_type_id_mask\":127,";
  ...
  dec_header += "\"chain_domain\":\"AZT1-CHAIN-V1-NONCE\",";
  ...
  dec_header += "\"stream_termination_api\":\"POST /api/v0/device/stream/terminate with stream_auth_nonce + reason_code + message_json + signer fields\",";
  dec_header += "\"stream_termination_signature_message\":\"stream_terminate:<nonce>:<device_sign_fp>:<reason_code>:<sha256_hex(canonical_message_json)>\",";
  dec_header += "\"message_attribution_fields\":[\"signed_by_fingerprint_hex\",\"signed_by_role\",\"user_message_sha256_hex\"],";
  ...
  dec_header += "\"For each chunk record read seq_u32be, block_flags_type_u8, body_len_u32be, tag_len_u8, body, tag, chain_v32.\",";
  dec_header += "\"Let CORE = seq_u32be||block_flags_type_u8||body_len_u32be||tag_len_u8||body||tag.\",";
  dec_header += "\"Let NONCE_HASH = SHA256(stream_auth_nonce_utf8) from outer header.\",";
  dec_header += "\"If seq==1: V_calc = SHA256(\\\"AZT1-CHAIN-V1-NONCE\\\"||NONCE_HASH||CORE).\",";
  dec_header += "\"If seq>1: V_calc = SHA256(\\\"AZT1-CHAIN-V1-NONCE\\\"||NONCE_HASH||V_prev||CORE).\",";
  dec_header += "\"Require V_calc == chain_v32; then set V_prev = chain_v32.\",";
  dec_header += "\"Derive is_encrypted=(block_flags_type_u8 & 0x80)!=0 and type_id=(block_flags_type_u8 & 0x7F). If is_encrypted, derive nonce as audio_nonce_prefix_b64 (4 bytes) || seq_u32be || 0x00000000 and AES-GCM decrypt body||tag with aad=none and require tag_len=16; otherwise require tag_len=0.\"";
  dec_header += "],";
  ...
  dec_header += "\"block_flags_type_u8 uses bit7 as encryption flag and low 7 bits as logical type id.\",";
  dec_header += "\"When bit7 is set, body is AES-GCM encrypted and tag_len must be 16; when clear, body is plaintext and tag_len must be 0.\",";
  ...
  plain_header += "\"chunk_record_format\":\"seq_u32be|block_flags_type_u8|body_len_u32be|tag_len_u8|body|tag|chain_v32\",";
  plain_header += "\"chain_alg\":\"sha256-link\",";
  plain_header += "\"chain_domain\":\"AZT1-CHAIN-V1-NONCE\",";
  plain_header += "\"chain_root_mode\":\"genesis-signature-block\",";
  plain_header += "\"chain_record_bytes_format\":\"seq_u32be|block_flags_type_u8|body_len_u32be|tag_len_u8|body|tag\",";
  plain_header += "\"chain_excludes_field\":\"chain_v32\",";
  plain_header += "\"chain_link_formula\":[";
  plain_header += "\"Compute NONCE_HASH = SHA256(stream_auth_nonce_utf8) from outer header.\",";
  plain_header += "\"For seq==1: chain_v32 = SHA256(chain_domain_bytes || NONCE_HASH || record_bytes).\",";
  plain_header += "\"For seq>1: chain_v32 = SHA256(chain_domain_bytes || NONCE_HASH || prev_chain_v32 || record_bytes).\"";
  plain_header += "],";
  plain_header += "\"block_type_encoding\":\"msb_encryption_flag_v1\",";
  plain_header += "\"block_type_encrypted_mask\":128,";
  ...
  plain_header += "\"stream_termination_api\":\"POST /api/v0/device/stream/terminate with stream_auth_nonce + reason_code + message_json + signer fields\",";
  plain_header += "\"stream_termination_signature_message\":\"stream_terminate:<nonce>:<device_sign_fp>:<reason_code>:<sha256_hex(canonical_message_json)>\",";
  plain_header += "\"message_attribution_fields\":[\"signed_by_fingerprint_hex\",\"signed_by_role\",\"user_message_sha256_hex\"],";
